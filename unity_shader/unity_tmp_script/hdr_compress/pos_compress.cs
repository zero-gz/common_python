...
public class pos_compress : MonoBehaviour {
    ...
    public Color RGBE_Encode(Color exr_color)
    {
        ...
        float E = Mathf.Clamp01((Mathf.Log(MaxColor, 2.0f)+128.0f)/256.0f);
        float Decode_MaxColor = Mathf.Pow(2.0f, E * 256.0f - 128.0f);
    ...
    public Color RGBE_Decode(Color rgbe_color)
    {
        ...
        float Decode_MaxColor = Mathf.Pow(2.0f, E*256.0f - 128.0f);
        Debug.Log(string.Format("get decode max color {0} {1}", Decode_MaxColor, E * 256.0f - 128.0f));
    ...
    public Color RGBM_Encode(Color exr_color, float MaxValue)
    {
        ...
        float Decode_MaxColor = Mathf.CeilToInt(M*256)/256.0f*MaxValue;
    ...
    public Color LOGUV_Encode(Color exr_color)
    {
        Color ret_color = new Color();
        ret_color = exr_color;
        return ret_color;
    ...
    {
       util.save_texture(save_path, encode_tex);
    }
