...
static TAutoConsoleVariable<float> CVarGRayTracingGlobalIllumiantionAOIntensity(
	TEXT("r.RayTracing.GlobalIllumination.AoIntensity"),
	2,
	TEXT("Normal bias for ray tracing global illumination (default = 2.0)"),
	ECVF_RenderThreadSafe);
...
class FRayTracingGlobalIlluminationSceneColorCompositePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		RENDER_TARGET_BINDING_SLOTS()
		SHADER_PARAMETER(uint32, NecaGlobalIllumination)
...
void FDeferredShadingSceneRenderer::RenderDiffuseIndirectAndAmbientOcclusion(FRHICommandListImmediate& RHICmdListImmediate)
{
	...
	for (FViewInfo& View : Views)
	{
		...
		const bool bApplyRTAO = ShouldRenderRayTracingAmbientOcclusion(View) && Views.Num() == 1 && View.FinalPostProcessSettings.RayTracingGIType != ERayTracingGlobalIlluminationType::NetEase; //#dxr_todo: enable RTAO in multiview mode
		...
		if (bApplyRTGI)
		{
			bool bIsValid = RenderRayTracingGlobalIllumination(GraphBuilder, SceneTextures, View, /* out */ &RayTracingConfig, /* out */ &DenoiserInputs, /* out */GlobalIlluminationRT);
			if (!bIsValid)
			{
				DenoiseMode = 0;
			}
		...
		if (DenoiseMode != 0 && View.FinalPostProcessSettings.RayTracingGIType != ERayTracingGlobalIlluminationType::NetEase)
		{
			const IScreenSpaceDenoiser* DefaultDenoiser = IScreenSpaceDenoiser::GetDefaultDenoiser();
			const IScreenSpaceDenoiser* DenoiserToUse = DenoiseMode == 1 ? DefaultDenoiser : GScreenSpaceDenoiser;
		...
		if ((DenoiserOutputs.Color || DenoiserOutputs.AmbientOcclusionMask) && View.FinalPostProcessSettings.RayTracingGIType != ERayTracingGlobalIlluminationType::NetEase)
		{
		...
		else if ((DenoiserInputs.Color || bApplySSAO) && View.FinalPostProcessSettings.RayTracingGIType == ERayTracingGlobalIlluminationType::NetEase)
		{
			...
			PassParameters->NecaGlobalIllumination = 1;
			PassParameters->AOIntensity = CVarGRayTracingGlobalIllumiantionAOIntensity.GetValueOnRenderThread();
			...
			PassParameters->AOTexture = GraphBuilder.RegisterExternalTexture(SceneContext.ScreenSpaceAO);
			PassParameters->AOSampler = TStaticSamplerState<SF_Point, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
			...
			GraphBuilder.AddPass(
				...
				PassParameters,
				ERDGPassFlags::Raster,
				[this, &SceneContext, &View, PixelShader, PassParameters](FRHICommandListImmediate& RHICmdList)
			{
				RHICmdList.SetViewport(View.ViewRect.Min.X, View.ViewRect.Min.Y, 0.0f, View.ViewRect.Max.X, View.ViewRect.Max.Y, 0.0);
	...
	for (FViewInfo& View : Views)
	{
		...
		if (bRayTracedReflections || bScreenSpaceReflections)
		{
			...
			if (bRayTracedReflections)
			{
				...
				if (View.FinalPostProcessSettings.RayTracingReflectionNeca)
				{
					RenderRayTracingNecaReflections(GraphBuilder, View, SceneContext.GIRadianceGather[SceneContext.GIRadianceCurrent][0], SceneContext.GIDepthGather[SceneContext.GIRadianceCurrent][0], &DenoiserInputs);
				}
				else
				{
			...
			if (bDenoise)
			{
				if (View.FinalPostProcessSettings.RayTracingReflectionNeca)
				{
					const IScreenSpaceDenoiser* DefaultDenoiser = IScreenSpaceDenoiser::GetNecaDenoiser();

					IScreenSpaceDenoiser::FReflectionsOutputs DenoiserOutputs = DefaultDenoiser->DenoiseReflections(
						GraphBuilder,
						View,
						&View.PrevViewInfo,
						SceneTextures,
