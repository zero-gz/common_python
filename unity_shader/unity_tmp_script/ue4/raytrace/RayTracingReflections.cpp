...
static int32 GRayTracingReflectionsNecaGILighting = 1;
static FAutoConsoleVariableRef CVarGRayTracingReflectionsNecaGILighting(
	TEXT("r.RayTracing.Reflections.NecaGILighting"),
	GRayTracingReflectionsNecaGILighting,
	TEXT("Enables ray tracing reflections direct lighting (default = 1)")
);
...
class FRayTracingNecaReflectionsRGS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FRayTracingNecaReflectionsRGS)
	SHADER_USE_ROOT_PARAMETER_STRUCT(FRayTracingNecaReflectionsRGS, FGlobalShader)
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, ColorDepthOutput)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, DistanceVectorPdfOutput)
		END_SHADER_PARAMETER_STRUCT()
...
IMPLEMENT_GLOBAL_SHADER(FRayTracingNecaReflectionsRGS, "/Engine/Private/RayTracing/RayTracingNecaReflections.usf", "RayTracingNecaReflectionsRGS", SF_RayGen);
...
bool ShouldRenderRayTracingReflections(const FViewInfo& View)
{
	...
	const bool bReflectionPassEnabled = ((ForceAllRayTracingEffects > 0 || (bReflectionsCvarEnabled && ForceAllRayTracingEffects < 0)) && (GetRayTracingReflectionsSamplesPerPixel(View) > 0)) || View.FinalPostProcessSettings.RayTracingReflectionNeca;
...
void FDeferredShadingSceneRenderer::PrepareRayTracingNecaReflections(const FViewInfo& View, TArray<FRHIRayTracingShader*>& OutRayGenShaders)
{
	// Declare all RayGen shaders that require material closest hit shaders to be bound
	...
	FRayTracingNecaReflectionsRGS::FPermutationDomain PermutationVector;
	PermutationVector.Set<FRayTracingNecaReflectionsRGS::FDeferredMaterialMode>(DeferredMaterialMode);

	auto RayGenShader = View.ShaderMap->GetShader<FRayTracingNecaReflectionsRGS>(PermutationVector);
...
void FDeferredShadingSceneRenderer::RenderRayTracingNecaReflections(
	FRDGBuilder& GraphBuilder,
	const FViewInfo& View,
	TRefCountPtr<IPooledRenderTarget>& GIRadianceGatherRT,
	TRefCountPtr<IPooledRenderTarget>& GIDepthGatherRT,
	IScreenSpaceDenoiser::FReflectionsInputs* OutDenoiserInputs)
#if RHI_RAYTRACING
{
	const uint32 SortTileSize = CVarRayTracingReflectionsSortTileSize.GetValueOnRenderThread();
...

	{
		...
		OutDenoiserInputs->Color = GraphBuilder.CreateTexture(Desc, TEXT("RayTracingNecaReflections"));
		OutDenoiserInputs->RayHitDistance = GraphBuilder.CreateTexture(Desc, TEXT("RayTracingReflectionsNecaHitDistance"));
	}
	...
	int32 ProbeSideLengthIrradiance = 8;
	int32 ProbeSideLengthDepth = 16;
	FIntVector ProbeCountInt = FIntVector(32, 32, 16);
	...
	ProbeCountInt = View.FinalPostProcessSettings.RayTracingGINecaProbeCount;
	FVector ProbeCount = FVector(ProbeCountInt.X, ProbeCountInt.Y, ProbeCountInt.Z);
	FVector ProbeStep = (EndPosition - StartPosition) / (ProbeCount - 1);
	int IrradianceWidth = (ProbeSideLengthIrradiance + 2) * ProbeCount.X * ProbeCount.Z + 2;
	int IrradianceHeight = (ProbeSideLengthIrradiance + 2) * ProbeCount.Y + 2;
	int DepthWidth = (ProbeSideLengthDepth + 2) * ProbeCount.X * ProbeCount.Z + 2;
	int DepthHeight = (ProbeSideLengthDepth + 2) * ProbeCount.Y + 2;

	FRayTracingNecaReflectionsRGS::FParameters CommonParameters;
	...
	CommonParameters.HeightFog = GRayTracingReflectionsHeightFog;
	CommonParameters.ShouldDoDirectLighting = GRayTracingReflectionsDirectLighting;
	CommonParameters.ShouldDoNecaGILighting = GRayTracingReflectionsNecaGILighting;
	CommonParameters.ReflectedShadowsType = 1;
	CommonParameters.ShouldDoEmissiveAndIndirectLighting = GRayTracingReflectionsEmissiveAndIndirectLighting;
	CommonParameters.UpscaleFactor = UpscaleFactor;
	CommonParameters.ReflectionMinRayDistance = FMath::Min(GRayTracingReflectionsMinRayDistance, GRayTracingReflectionsMaxRayDistance);
	CommonParameters.ReflectionMaxRayDistance = GRayTracingReflectionsMaxRayDistance;
	...
	CommonParameters.WorldGIStartPosition = StartPosition;
	CommonParameters.WorldGIProbeCount = ProbeCount;
	CommonParameters.ProbeStep = ProbeStep;
	CommonParameters.ProbeSideLengthIrradiance = ProbeSideLengthIrradiance;
	CommonParameters.ProbeSideLengthDepth = ProbeSideLengthDepth;
	CommonParameters.NormalBias = 25;
	CommonParameters.EnergyPreservation = 0.85;
	CommonParameters.TextureSizeIrradiance = FVector4(IrradianceWidth, IrradianceHeight, 1.0f / IrradianceWidth, 1.0f / IrradianceHeight);
	CommonParameters.TextureSizeDepth = FVector4(DepthWidth, DepthHeight, 1.0f / DepthWidth, 1.0f / DepthHeight);
	
	if (GIRadianceGatherRT && GIDepthGatherRT)
	{
		CommonParameters.RadianceTexture = GraphBuilder.RegisterExternalTexture(GIRadianceGatherRT);
		CommonParameters.DepthTexture = GraphBuilder.RegisterExternalTexture(GIDepthGatherRT);
	}
	else
	{
		CommonParameters.RadianceTexture = GraphBuilder.RegisterExternalTexture(GSystemTextures.BlackDummy);
		CommonParameters.DepthTexture = GraphBuilder.RegisterExternalTexture(GSystemTextures.WhiteDummy);
	}
	CommonParameters.RadianceSampler = TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
	CommonParameters.DepthSampler = TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
	...
	CommonParameters.ReflectionStruct = CreateReflectionUniformBuffer(View, EUniformBufferUsage::UniformBuffer_SingleFrame);
	CommonParameters.FogUniformParameters = CreateFogUniformBuffer(View, EUniformBufferUsage::UniformBuffer_SingleFrame);
	CommonParameters.ColorDepthOutput = GraphBuilder.CreateUAV(OutDenoiserInputs->Color);
	CommonParameters.DistanceVectorPdfOutput = GraphBuilder.CreateUAV(OutDenoiserInputs->RayHitDistance);
	CommonParameters.SortTileSize = SortTileSize;

	for (uint32 PassIndex = 0; PassIndex < NumPasses; ++PassIndex)
	{
		FRayTracingNecaReflectionsRGS::FParameters* PassParameters = GraphBuilder.AllocParameters<FRayTracingNecaReflectionsRGS::FParameters>();
		*PassParameters = CommonParameters;
		...
		PassParameters->BlueNoiseTexture = GEngine->BlueNoiseTexture->Resource->TextureRHI;
